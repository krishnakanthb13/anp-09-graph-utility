...
export async function handleDownload(app, noteUUID) {
  try {
    ...
    const note = (typeof app.findNote === "function")
      ? await app.findNote({ uuid: noteUUID })
      : (app.notes && typeof app.notes.find === "function" ? await app.notes.find(noteUUID) : { name: "Untitled Note", tags: [] });

    const fullNoteContent = `
Note Name: ${note ? note.name : "Untitled Note"}
Note Tags: ${note && note.tags ? note.tags.join(", ") : ""}
Note UUID: ${noteUUID}
