...
export function tokenizeMath(input) {
  ...
  while (i < len) {
    ...
    // Numbers (integers, floating point, and scientific notation e.g. 1e3, 1.2e-4, 2.5E+6)
    if (/[0-9]/.test(ch) || (ch === "." && i + 1 < len && /[0-9]/.test(raw[i + 1]))) {
      ...
      // Check for scientific notation exponent (e or E)
    ...
    case "BINARY_OP": {
      ...
      switch (node.operator) {
        ...
        case "*":
          ...
          break;
        case "/":
          if (right === 0) return null; // Avoid division by zero singularity
          res = left / right;
          break;
        case "%":
