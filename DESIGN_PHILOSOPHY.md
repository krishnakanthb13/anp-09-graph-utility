...
- **Flexible Observation Mapping & Natural Order Ergonomics**: Recognizing that not every dataset has a dedicated category/date column. Adding optional X-Axis (`Remove from X` / `Auto Index (No X)`) automatically frees Column 1 and naturally places it at the beginning of the series list, treating all columns as first-class numerical variables.
...
- **Aspect Ratio Freedom (Standard Measured vs Stretched)**: Some charts require fixed proportions (e.g. radial polar diagrams or precision time series) while others shine when expanding to fill wide ultra-wide monitors. Providing a 1-click toggle between Standard Measured and Stretched Sizing empowers users to choose the optimal layout for their data.
- **Progressive Disclosure & Collapsible Controls**: Advanced customization controls (grid lines, smoothing, legends, data labels) should be available on demand without overwhelming the primary workflow. Collapsible accordions maintain visual serenity while keeping full configuration accessible.
- **Per-Series Precision Color Customization**: Every dataset has semantic meaning (e.g., revenue in emerald green, expenses in red, brand primary colors). Allowing individual color overrides with decoupled selection interactions turns generic charts into customized executive reports.
- **Visual Layering, Reordering & Instant Reset Hierarchy**: Data points that need prominence must be easily brought to the foreground. Move Up/Down reordering gives users complete control over chart Z-index layering, bar column grouping sequence, and legend priority, with an instant `Reset Order` button to snap back to the table's native column sequence in 1 click.
